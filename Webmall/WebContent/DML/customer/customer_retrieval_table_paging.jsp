...
<head>
<meta charset="UTF-8">
<title>고객정보 테이블 검색과 페이지 제어</title>
...
<%
	...
	int start_pointer = 0;			// DB 검색 시작위치
	...
	//JDBC 드라이버 로딩 & MariaDB 서버와 데이터베이스연결
...
//****************************************************************
	...
	//질의처리 결과체크
	//cursor - 초기상태 아니거나 행이 없이면 false 리턴
	if(!rset.isBeforeFirst()) {
		out.print("<script>alert('고객테이블이 비어 있습니다.');" + "history.back();" 
					+ "</script>");
	}
...
<td colspan="8" style="background-color:white; text-align:right; 
color:blue;">현재 회원 수 <%=new DecimalFormat("#,###").format(nbr_of_row)%> 명&nbsp; (전체 <%=nbr_of_page%> 쪽 중&nbsp;현재 <%=cur_page_no%> 쪽)</td>
</tr>
